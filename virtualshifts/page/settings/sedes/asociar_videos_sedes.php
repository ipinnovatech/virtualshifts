...
$objVideosSedes->get_videos_por_sede($id_sede);
$array_videos = $objVideosSedes->array_campos;

$objVideos->get_total_videos_activos($_SESSION['cliente']);
$total_videos = $objVideos->result;
...
<head>
    ...
    <script type="text/javascript">
        var total_seleccionados = <?php echo count($array_videos); ?>;
        var total_areas = <?php echo $total_videos; ?>;
        ...
        function guardar_videos_sedes(){
            ...
            $.ajax({
                type: "POST",
                url: 'agregar_videos_por_sede.php',
                data: form_videos,
        ...
        <div class="page-region">
            <div class="page-region-content">                
                <h1>
                    ...
                    Asociar<small class="on-right">Videos a sedes</small>
                </h1>
                
                <div class="span10">
                <span style="font-size: 20px;">Videos seleccionadas:&nbsp;<span id="cantidad_seleccion"><?php echo count($array_videos); ?></span> de <?php echo $total_videos; ?></span>
                ...
                <form id="form_videos" action="javascript: guardar_videos_sedes();">
                    ...
                    <div id="todos_videos" class="tile-group six" style="border: 1px; border-color: #999999; border-style: solid; padding-top: 0px !important; overflow: auto; height: 500px;">
                        <?php
                            $objVideos->get_videos_activos($_SESSION['cliente']);
                            if($objVideos->has_value){
                                foreach($objVideos->array_videos as $row){ ?>
                                    <div class="tile double bg-white live <?php echo (in_array($row['VID_ID'],$array_videos))?'selected':'' ?>" id="video_<?php echo $row['VID_ID']; ?>" onclick="seleccionar_video('<?php echo $row['VID_ID']; ?>')" style="margin: 5px;">
                                        <div class="tile-content image">
                                            <video class="padding10" muted="muted" width="250" loop="loop" height="110" autoplay="autoplay">
                                                <source src="<?php echo $row['VID_URL'] ?>" />
                                            </video>
                                        </div>
                                        <?php if(in_array($row['VID_ID'],$array_videos)){ ?>
                                            <input class="video_hidden" type="hidden" name="videos[]" value="<?php echo $row['VID_ID']; ?>" />
                                        <?php } ?>
                                        <input type="hidden" id="id_video" value="<?php echo $row['VID_ID']; ?>" />
                                    </div>
                                <?php
                                }
                            }
